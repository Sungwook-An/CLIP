...
def contrastive_loss(args, image_embeds, text_embeds, labels, logit_scale):
    ...
    logits_per_text = logit_scale * torch.matmul(text_embeds, image_embeds.t())
    ...
    with torch.no_grad():
        if args.local_rank == 0:
            ...
            for images, texts, labels in tqdm(val_loader):
                image_embeddings_all, text_embeddings_all, labels_all, _ = val_one_epoch(args, image_encoder, text_encoder, image_projection, text_projection, image_embeddings_all, text_embeddings_all, labels_all, images, texts, labels)
        else:
...
def save_checkpoint(state, is_best, filename='/root/LGD2024/examples_old/imagenet/clip_ckpt/last.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, '/root/LGD2024/examples_old/imagenet/clip_ckpt/model_best.pth.tar')
...
def main_worker(args):
    ...
    image_encoder = EfficientNet.from_pretrained(args.arch, weights_path='/root/LGD2024/examples_old/imagenet/model_best_blurpool_78_528.pth.tar', advprop=args.advprop)
    ...
    if args.distributed:
        ...
        val_sampler = distributed.DistributedSampler(dataset=val_dataset, shuffle=True)
    else:
        train_sampler = None
        val_sampler = None
    ...
    if args.distributed:
        ...
        val_loader = DataLoader(val_dataset, batch_size=args.ft_batch_size, shuffle=True, num_workers=args.workers, collate_fn=collate_fn, pin_memory=True)
